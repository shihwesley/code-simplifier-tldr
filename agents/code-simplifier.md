...
### 1. Merkle-First Check

Check if mercator manifest exists for instant change detection:
```bash
# O(1) check: has anything changed since last mapping?
ls docs/.mercator.json 2>/dev/null
```

If the manifest exists and you're looking at recently mapped files, their purposes are already known — read `docs/CODEBASE_MAP.md` for pre-analyzed architecture.
...
1. "Let me check what files changed recently"
   → git diff --name-only HEAD~3
   → Shows: changed: ["src/api/auth.ts", "src/api/users.ts"]
   "Two files changed - I'll focus there"

2. "I'll get TLDR for the changed files"
